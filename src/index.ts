...
if (!CONFIG.disable_integrations) {
  const getVersionFile = () => {
    const versionFile: string = path.join(import.meta.dirname, '../../version.json')
    try {
      return JSON.parse(
      ...
      console.warn(
        `Version file ${versionFile} not found. This is normal on from-source builds.`
      );
      return {};
    }
  };
  ...
  app.get('/endpoints', (_,res) => {
    res.send({...getEndPoints(), config: undefined})
  })
  app.get('/info', (_, res) => {
...
server.listen(CONFIG.port, async () => {
  ...
  if (CONFIG.widget_list.includes('network')) {
    try {
      ...
      // TODO: integrate Universal Speedest
      //
